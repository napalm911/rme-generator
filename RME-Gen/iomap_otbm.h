...
public:
	...
	static bool getVersionInfo(const wxFileName& identifier, MapVersion& out_ver);

	virtual bool loadMap(Map& map, const wxFileName& identifier);
	virtual bool saveMap(Map& map, const wxFileName& identifier);

protected:
	static bool getVersionInfo(NodeFileReadHandle* f,  MapVersion& out_ver);

	virtual bool loadMap(Map& map, NodeFileReadHandle& handle);
	bool loadSpawns(Map& map, const wxFileName& dir);
	bool loadSpawns(Map& map, pugi::xml_document& doc);
	bool loadHouses(Map& map, const wxFileName& dir);
	bool loadHouses(Map& map, pugi::xml_document& doc);

	virtual bool saveMap(Map& map, NodeFileWriteHandle& handle);
	bool saveSpawns(Map& map, const wxFileName& dir);
	bool saveSpawns(Map& map, pugi::xml_document& doc);
	bool saveHouses(Map& map, const wxFileName& dir);
